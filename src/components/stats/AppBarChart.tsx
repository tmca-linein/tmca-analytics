...
function AppBarChart<TData extends Record<string, string | number>>(
    props: AppBarChartProps<TData>,
) {
    const {
        chartTitle,
        chartDesc,
        ...
        <div className="flex-1">
            <Card className="pt-0">
                    ...
                    <div className="flex gap-2 sm:ml-auto">
                        {visibleFilters.map((f) => (
                            <div key={f.id} className="flex gap-2">
                                ...
                                >
                                    <SelectTrigger
                                        className={f.triggerClassName ?? "hidden w-[180px] rounded-lg sm:flex"}
                                        aria-label={f.label}
                                    >
                ...
                <CardContent className="px-2 pt-4 sm:px-6 sm:pt-6">
                    <ChartContainer config={chartConfig} className={`${heightClassName} w-full`}>
                        <BarChart
                            accessibilityLayer
                            data={filteredData}
                        ...
                        >
                            ...
                            {series.map((s) => (
                                <Bar
                                    ...
                                    fill={`var(--color-${s.key as string})`}
                                    radius={[6, 6, 0, 0]}
                                />
                            ))}
                        </BarChart>
